...
def generate_feature_metadata_documentation() -> None:
    if FEATURE_METADATA_DOCUMENTATION_PATH.exists():
        existing_df = pd.read_csv(FEATURE_METADATA_DOCUMENTATION_PATH, nrows=1)
        if "registry_name" in existing_df.columns:
            print(
                "Keeping feature store metadata documentation generated by "
                "src.feature_store.retrieve_retailrocket_features."
            )
            return
...
def generate_dvc_versioning_summary() -> None:
    dvc_files = [
        ...
        {
            "pipeline_layer": "raw_batch_data",
            "description": "Retailrocket raw batch ingestion snapshots",
            "file": Path("data/raw/source=retailrocket_batch.dvc"),
        },
        {
            "pipeline_layer": "raw_api_data",
            "description": "Retailrocket raw REST API catalog delta snapshots",
            "file": Path("data/raw/source=retailrocket_api.dvc"),
        },
        {
        ...
        {
            "pipeline_layer": "curated_data",
            "description": "Retailrocket curated analytical Parquet data",
            "file": Path("data/curated/source=retailrocket.dvc"),
        },
        {
